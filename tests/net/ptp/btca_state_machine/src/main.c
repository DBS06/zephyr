...
ZTEST(ptp_btca_state_machine, test_state_machine_time_transmitter_to_grand_master)
{
	enum ptp_port_state new_state;

	new_state = ptp_state_machine(PTP_PS_TIME_TRANSMITTER, PTP_EVT_RS_GRAND_MASTER, false);
	zassert_equal(new_state, PTP_PS_GRAND_MASTER, "unexpected state: %d", new_state);
}
