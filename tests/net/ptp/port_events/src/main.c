...
static struct ptp_tlv fake_added_tlv;
...
ZTEST(ptp_port_events, test_management_set_delay_mechanism_and_pdelay_interval)
{
	...
	zassert_ok(ptp_port_management_msg_process(&port, &port, &req, tlv),
		   "delay mechanism SET failed");
	zassert_equal(port.port_ds.delay_mechanism, PTP_DM_P2P, "delay mechanism not updated");
	zassert_equal(port.port_ds.mean_link_delay, 0, "PDelay measurement state not reset");
	zassert_false(port.neighbor_rate_ratio_valid, "neighbor rate ratio should be reset");
	zassert_equal(transport_send_calls, 1, "SET response not sent");
	stop_port_timers(&port);
	...
	zassert_equal(transport_send_calls, 1, "interval SET response not sent");
	stop_port_timers(&port);
}

ZTEST(ptp_port_events, test_management_rejects_unsupported_delay_mechanism)
{
	uint8_t storage[sizeof(struct ptp_tlv_mgmt) + 2] = {0};
	struct ptp_tlv_mgmt *tlv = (struct ptp_tlv_mgmt *)storage;
	struct ptp_port port;
	...
	tlv->id = PTP_MGMT_DELAY_MECHANISM;
	tlv->length = sizeof(tlv->id) + sizeof(uint8_t);
	tlv->data[0] = PTP_DM_COMMON_P2P;

	zassert_ok(ptp_port_management_msg_process(&port, &port, &req, tlv),
		   "invalid delay mechanism should produce management error");
	zassert_equal(port.port_ds.delay_mechanism, PTP_DM_E2E,
		      "invalid mechanism should not update port");
	zassert_equal(transport_send_calls, 1, "management error not sent");
	zassert_equal(last_tx_msg.header.type_major_sdo_id, PTP_MSG_MANAGEMENT,
		      "unexpected management error message type");
	stop_port_timers(&port);
}
