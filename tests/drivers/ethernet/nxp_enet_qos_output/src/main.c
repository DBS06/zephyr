...
ZTEST(nxp_enet_qos_output, test_gate_window_and_clock_movement)
{
	...
	decision = ptp_clock_nxp_enet_qos_output_gate_decide(target - 500 * NSEC_PER_MSEC - 1,
							      target);
	zassert_equal(decision.action, PTP_CLOCK_NXP_ENET_QOS_OUTPUT_GATE_WAIT);
	zassert_equal(decision.delay_ns, 1);

	decision = ptp_clock_nxp_enet_qos_output_gate_decide(target - 500 * NSEC_PER_MSEC,
							      target);
	...
	decision = ptp_clock_nxp_enet_qos_output_gate_decide(target - 20 * NSEC_PER_MSEC,
							      target);
	zassert_equal(decision.action, PTP_CLOCK_NXP_ENET_QOS_OUTPUT_GATE_ROUTE);
	...
	decision = ptp_clock_nxp_enet_qos_output_gate_decide(target - 750 * NSEC_PER_MSEC,
							      target);
	zassert_equal(decision.action, PTP_CLOCK_NXP_ENET_QOS_OUTPUT_GATE_WAIT);
	zassert_equal(decision.delay_ns, 250 * NSEC_PER_MSEC);
