...
static const struct precision_time_domain test_domain = {
	.type = PRECISION_TIME_DOMAIN_PHC,
	.id = 99,
};

static const struct precision_time_domain test_domain_other = {
	.type = PRECISION_TIME_DOMAIN_PHC,
	.id = 100,
};
...
static int fake_read(const struct precision_clock *clock, struct precision_time_point *tp)
{
	struct fake_clock *f = (struct fake_clock *)clock->adapter;
	...
	*tp = (struct precision_time_point){
...
static void fake_init(struct fake_clock *f, const struct precision_time_domain *domain)
{
	k_mutex_lock(&fake_lock, K_FOREVER);
	memset(f, 0, sizeof(*f));
	f->clock.api = &fake_api;
	f->clock.adapter = f;
	f->clock.domain = *domain;
...
{
	k_mutex_lock(&fake_lock, K_FOREVER);
	f->status.config.waveform.first_rising_time.time += 1;
	k_mutex_unlock(&fake_lock);
}
...
ZTEST(precision_timing_pps_output, test_initial_arm_stops_then_starts_fresh_output)
{
	...
	zassert_equal(rec_a.state.config.first_rising_time.time % (precision_time_t)NSEC_PER_SEC,
		      0);
	zassert_true(precision_time_domain_equal(&rec_a.state.config.first_rising_time.domain,
						 &test_domain));
...
ZTEST(precision_timing_pps_output, test_adopts_already_active_matching_output)
{
	...
	fake.status.config.waveform = (struct precision_clock_output_waveform_config){
		.first_rising_time = {.time = 0, .domain = test_domain},
...
ZTEST(precision_timing_pps_output, test_callback_snapshot_matches_state_get)
{
	...
	zassert_equal(queried.config.first_rising_time.time,
		      rec_a.state.config.first_rising_time.time);
...
ZTEST(precision_timing_pps_output, test_two_instances_run_and_stop_independently)
{
	...
	fake_init(&fake, &test_domain);
	fake_init(&fake_other, &test_domain_other);
	...
	zassert_ok(precision_pps_output_state_get(&pps, &state));
	zassert_true(
		precision_time_domain_equal(&state.config.first_rising_time.domain, &test_domain));
	zassert_ok(precision_pps_output_state_get(&pps_other, &state));
	zassert_true(precision_time_domain_equal(&state.config.first_rising_time.domain,
						 &test_domain_other));
