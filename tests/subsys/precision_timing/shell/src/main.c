...
{
	return (struct fake_clock *)clock->data;
}

static int fake_clock_read(const struct precision_clock *clock, precision_time_t *time)
{
...
static void fake_clock_init(struct fake_clock *fake, precision_time_t time)
{
...
ZTEST(precision_timing_shell, test_output_event_scheduling_forwarding_and_errors)
{
	...
	/* A falling one-shot event forwards the target time and edge. */
	zassert_ok(execute("precision_clock output event clock 1 5000000000 falling"));
	zassert_equal(fake.schedule_event_calls, 1);
	zassert_equal(fake.last_output_channel, 1);
	zassert_equal(fake.last_event_config.target_time, 5000000000LL);
	...
	assert_output_contains(output, "target_time_ns=5000000000 edge=falling");
	...
	zassert_equal(fake.last_event_config.target_time, -2000000000LL);
