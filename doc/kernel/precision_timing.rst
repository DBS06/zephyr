...
is 20 nanoseconds, and the maximum period or width is about 85.9 seconds.
...
the original lead time cannot delay the following pulse. This single-pulse
rearming approach has been validated on hardware, although it differs from
ST's documented coarse-correction workaround.

The MCXN947 ENET QoS provider exposes one fixed 1 Hz PPS output when
:kconfig:option:`CONFIG_PTP_CLOCK_NXP_ENET_QOS_OUTPUT` is enabled. The hardware
does not provide programmable interval or width registers, so the provider
accepts only the provider-default width. Each pulse is one 50 MHz PTP
system-time clock period, nominally 20 nanoseconds, which is substantially
narrower than a conventional millisecond-scale PPS pulse. The provider routes
the physical pin 500 milliseconds before a requested integral-second edge;
requests made inside that window are routed synchronously. The board pin route
is opt-in because the selected pad can be shared with other peripherals.
