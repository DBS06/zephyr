...
static int cmd_caps(const struct shell *shell, size_t argc, char **argv)
{
	...
	shell_print(shell, "read=%s set=%s adjust_phase=%s adjust_rate=%s scheduled_output=%s",
		    ...
		    (caps.flags & PRECISION_CLOCK_CAP_SCHEDULED_OUTPUT) != 0 ? "yes" : "no");
				...
				const struct precision_clock_output_status *status)
{
	...
	if (status->configured && status->kind == PRECISION_CLOCK_OUTPUT_KIND_EVENT) {
		...
		shell_print(shell, "target_time_ns=%lld domain=%s domain_id=%u edge=%s",
			    (long long)event->target_time.time,
			    domain_type_name(event->target_time.domain.type),
			    event->target_time.domain.id, edge_name(event->edge));
		...
		shell_print(shell, "first_rising_time_ns=%lld domain=%s domain_id=%u",
			    (long long)waveform->first_rising_time.time,
			    domain_type_name(waveform->first_rising_time.domain.type),
...
static int cmd_output_event(const struct shell *shell, size_t argc, char **argv)
{
	...
	ret = parse_u32(shell, "channel", argv[2], &channel);
	if (ret == 0) {
		ret = parse_time(shell, "target time", argv[3], &config.target_time.time);
	}
	if (ret == 0) {
	...
	ret = precision_timing_shell_operation_acquire(argv[1], &operation);
	if (ret == 0) {
		config.target_time.domain = operation.clock->domain;
...
static int cmd_output_waveform(const struct shell *shell, size_t argc, char **argv)
{
	...
	ret = parse_u32(shell, "channel", argv[2], &channel);
	if (ret == 0) {
		ret = parse_time(shell, "first rising time", argv[3],
	...
	ret = precision_timing_shell_operation_acquire(argv[1], &operation);
	if (ret == 0) {
		config.first_rising_time.domain = operation.clock->domain;
...
static int cmd_pps_start(const struct shell *shell, size_t argc, char **argv)
{
	...
	shell_print(shell, "first_rising_time_ns=%lld period_ns=%lld pulse_width_ns=%lld",
		    (long long)config.first_rising_time.time, (long long)config.period_ns,
