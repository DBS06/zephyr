...
{
	if (!precision_time_domain_equal(&waveform->first_rising_time.domain,
					 &pps->clock->domain)) {
		return false;
	}
	if ((waveform->first_rising_time.time % PPS_OUTPUT_PERIOD_NS) != 0) {
		return false;
	}
...
static bool continuity_hard_step(struct precision_pps_output *pps,
				 const struct precision_time_point *now,
				 precision_time_t monotonic_ns)
{
	...
	ret = precision_time_sub(now->time, pps->previous_phc_ns, &phc_elapsed);
	if (ret == 0) {
	...
	if (ret < 0) {
		pps->state.continuity_error_ns =
			now->time >= pps->previous_phc_ns ? PRECISION_TIME_MAX : PRECISION_TIME_MIN;
		return true;
	}
...
static void remember_read(struct precision_pps_output *pps, const struct precision_time_point *now,
			  precision_time_t monotonic_ns)
{
	pps->previous_phc_ns = now->time;
	pps->previous_monotonic_ns = monotonic_ns;
	pps->state.phc_time_ns = now->time;
...
{
	int ret = precision_clock_output_stop(pps->clock, pps->config.channel);
...
static int start_from_read(struct precision_pps_output *pps, const struct precision_time_point *now,
			   uint32_t *events)
{
	...
	if (ret == 0) {
		ret = precision_clock_output_start_waveform(pps->clock, pps->config.channel,
							    &waveform);
	}
	if (ret < 0) {
		pps->state.last_error = ret;
...
static void pps_output_poll_locked(struct precision_pps_output *pps, uint32_t *events)
{
	...
	ret = precision_clock_read(pps->clock, &now);
	read_ok = ret == 0;
	if (!read_ok) {
	...
	ret = precision_clock_output_get_status(pps->clock, pps->config.channel, &status);
	if (ret != 0) {
		pps->state.last_error = ret;
			      ...
			      precision_pps_output_callback_t callback, void *user_data)
{
	...
	if (pps == NULL || clock == NULL || config == NULL || callback == NULL) {
		return -EINVAL;
	}
...
int precision_pps_output_start(struct precision_pps_output *pps)
{
	...
	ret = precision_clock_output_get_caps(pps->clock, pps->config.channel, &pps->caps);
	if (ret == 0) {
...
int precision_pps_output_stop(struct precision_pps_output *pps)
{
	...
	ret = precision_clock_output_stop(clock, channel);
