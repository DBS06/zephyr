...
static const struct precision_pps_output_config pps_config = {
	.channel = PPS_CHANNEL,
				...
				const struct precision_pps_output_state *state, void *user_data)
{
	...
	if ((events & PRECISION_PPS_OUTPUT_EVENT_ARMED) != 0U) {
		LOG_INF("PPS service armed channel %u: first_rising_ns=%lld period_ns=%lld "
			"pulse_width_ns=%lld generation=%u (service state only; not a physical "
			"waveform check)",
			pps_config.channel, (long long)state->config.first_rising_time.time,
			(long long)state->config.period_ns, (long long)state->config.pulse_width_ns,
			state->generation);
	}
...
int main(void)
{
	const struct precision_time_domain phc_domain = {
	...
	ret = precision_clock_ptp_init(&phc_adapter, ptp_clock, phc_domain);
	if (ret < 0) {
	 ...
	 * does not verify a physical waveform on the STM32 output pin. This loop
