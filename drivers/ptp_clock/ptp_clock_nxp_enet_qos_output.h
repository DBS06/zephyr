...
/* Reserve time for pinctrl and verify completion before the target as well. */
#define PTP_CLOCK_NXP_ENET_QOS_OUTPUT_ROUTE_LEAD_NS  (1 * (precision_time_t)NSEC_PER_MSEC)
...
static inline int ptp_clock_nxp_enet_qos_output_caps(uint32_t channel,
	...
	uint32_t channel, const struct precision_clock_output_raw_waveform_config *config)
{
	...
	if (config->first_rising_time < 0 ||
	    ...
	    config->period_ns != PTP_CLOCK_NXP_ENET_QOS_OUTPUT_PERIOD_NS ||
					  ...
					  precision_time_t first_rising_time)
{
	...
	if (now_ns > first_rising_time - PTP_CLOCK_NXP_ENET_QOS_OUTPUT_ROUTE_LEAD_NS) {
		...
		decision.delay_ns =
			MIN(gate_time_ns - now_ns, PTP_CLOCK_NXP_ENET_QOS_OUTPUT_MAX_RECHECK_NS);
	} else {
