...
static int ptp_clock_nxp_enet_qos_get_unlocked(const struct device *dev, struct net_ptp_time *tm);

#if defined(CONFIG_PTP_CLOCK_NXP_ENET_QOS_OUTPUT)
static int ptp_clock_nxp_enet_qos_output_time_to_ns(const struct net_ptp_time *time,
						    precision_time_t *time_ns)
{
	...
	if (time->nanosecond >= NSEC_PER_SEC || time->second > seconds_limit ||
	    (time->second == seconds_limit && time->nanosecond > nanoseconds_limit)) {
		return -ERANGE;
	}

	*time_ns = (precision_time_t)time->second * NSEC_PER_SEC + time->nanosecond;
...
static int ptp_clock_nxp_enet_qos_output_route(const struct device *dev)
{
	...
	ret = ptp_clock_nxp_enet_qos_output_time_to_ns(&tm, &now_ns);
	if (ret < 0) {
		return ret;
	}
	decision = ptp_clock_nxp_enet_qos_output_gate_decide(now_ns,
							 data->output_config.first_rising_time);
	...
	/* Interrupts and hardware time continue while the pin is being routed. */
	ret = ptp_clock_nxp_enet_qos_get_unlocked(dev, &tm);
	if (ret < 0) {
		return ret;
	}
	...
	return now_ns >= data->output_config.first_rising_time ? -ETIME : 0;
}

static void ptp_clock_nxp_enet_qos_output_fail(struct ptp_clock_nxp_enet_qos_data *data, int error)
{
	int ret;
...
static int ptp_clock_nxp_enet_qos_output_reschedule(struct ptp_clock_nxp_enet_qos_data *data,
...
static void ptp_clock_nxp_enet_qos_output_cancel_work_sync(struct ptp_clock_nxp_enet_qos_data *data)
{
	...
	const struct precision_clock_output_raw_waveform_config *output_config)
{
	...
	if (output_config->first_rising_time < now_ns ||
	    output_config->first_rising_time - now_ns < PTP_CLOCK_NXP_ENET_QOS_OUTPUT_MIN_LEAD_NS) {
		ret = -ETIME;
		goto out;
	}
	...
	decision =
		ptp_clock_nxp_enet_qos_output_gate_decide(now_ns, output_config->first_rising_time);
	if (decision.action == PTP_CLOCK_NXP_ENET_QOS_OUTPUT_GATE_ROUTE) {
...
static int
ptp_clock_nxp_enet_qos_output_get_status(const struct device *dev, uint32_t channel,
...
#define PTP_CLOCK_NXP_ENET_QOS_INIT(n)                                                             \
	...
	static const struct ptp_clock_nxp_enet_qos_config ptp_clock_nxp_enet_qos_##n##_config = {  \
		...
		.clock_subsys = (clock_control_subsys_t)DT_CLOCKS_CELL_BY_NAME(DT_INST_PARENT(n),  \
									       ptp, name),         \
