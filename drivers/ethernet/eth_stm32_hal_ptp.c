...
struct ptp_clock_stm32_data {
	...
	struct precision_clock_output_raw_status output_status;
...
static int ptp_clock_stm32_output_start(
	...
	const struct precision_clock_output_raw_waveform_config *config)
{
	...
	ETH_HandleTypeDef *heth = &eth_dev_data->heth;
	uint32_t resolution_ns = ptp_clock_stm32_output_resolution_ns();
	struct net_ptp_time start_time;
	...
	if (config->period_ns <= 0 || config->pulse_width_ns <= 0 ||
	    config->pulse_width_ns >= config->period_ns) {
		return -EINVAL;
	}
	if (config->first_rising_time < 0 ||
	    config->first_rising_time / NSEC_PER_SEC > UINT32_MAX) {
		return -ERANGE;
	}

	start_time.second = (uint64_t)(config->first_rising_time / NSEC_PER_SEC);
	start_time.nanosecond = (uint32_t)(config->first_rising_time % NSEC_PER_SEC);
	if ((start_time.nanosecond % resolution_ns) != 0U) {
		return -ERANGE;
	}
	...
	if (data->output_status.configured) {
		ret = -EBUSY;
		goto unlock;
	}
	...
	if (!ptp_clock_stm32_output_has_lead_time(&now, &start_time)) {
		ret = -ETIME;
		irq_unlock(key);
		goto unlock;
	}
	...
	heth->Instance->MACPPSTTNR = start_time.nanosecond;
	heth->Instance->MACPPSTTSR = (uint32_t)start_time.second;
	...
	irq_unlock(key);
	ret = ptp_clock_stm32_output_wait_ready(heth);
	data->output_status = (struct precision_clock_output_raw_status){
		.configured = true,
		.kind = PRECISION_CLOCK_OUTPUT_KIND_WAVEFORM,
		.config.waveform = *config,
		.hardware_active_valid = true,
...
static int ptp_clock_stm32_output_stop(const struct device *dev, uint32_t channel)
{
	...
	data->output_status.configured = false;
	data->output_status.hardware_active = false;
...
static const struct precision_clock_output_provider ptp_clock_stm32_output_provider = {
	.get_caps = ptp_clock_stm32_get_output_caps,
	.start_waveform = ptp_clock_stm32_output_start,
	.stop = ptp_clock_stm32_output_stop,
	.get_status = ptp_clock_stm32_get_output_status,
};
#endif
...
static int ptp_stm32_init(const struct device *dev)
{
	...
	data->output_status = (struct precision_clock_output_raw_status){0};
